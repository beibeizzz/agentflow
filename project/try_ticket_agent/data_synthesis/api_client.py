...
class DeepSeekClient:
    ...
    def judge(self, source: dict[str, Any], user_request: str) -> JsonResponse:
        return self._complete_json(
            messages=build_judge_messages(source, user_request),
        ...
        *,
        ...
        for attempt in range(1, self.max_transport_attempts + 1):
            try:
                ...
                usage = add_usage(usage, extract_usage(response))
                ...
                return JsonResponse(payload, usage)
...
def add_usage(left: ApiUsage, right: ApiUsage) -> ApiUsage:
    return ApiUsage(
