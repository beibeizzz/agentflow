...
class Ticket_Query_Tool(BaseTool):
    require_llm_engine = False

    def __init__(self, model_string: str | None = None) -> None:
        super().__init__(
            tool_name=TOOL_NAME,
            tool_description="Find exactly one sandbox ticket by ticket_id, customer_id, or order_id.",
            tool_version="1.0.0",
            input_types={"lookup_by": "str", "value": "str"},
            output_type="dict",
            demo_commands=[],
    ...
    def execute(self, lookup_by: str, value: str) -> dict:
        if self.backend is None:
            return result(False, "BACKEND_NOT_BOUND", "Ticket backend is not bound.")
        return self.backend.query(lookup_by, value)
