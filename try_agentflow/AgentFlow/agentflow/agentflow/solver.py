...
class Solver:
    ...
    @staticmethod
    def _normalize_planner_text(value: Optional[str]) -> str:
        if value is None:
            return ""
        return re.sub(r"\s+", " ", str(value)).strip()

    @staticmethod
    def _normalize_calculation(value: Optional[str]) -> str:
        if value is None:
            return ""
        return re.sub(r"\s+", "", str(value))

    def _detect_repeated_planner_step(self, context: Optional[str], sub_goal: Optional[str]) -> Optional[str]:
        current_sub_goal = self._normalize_planner_text(sub_goal)
        current_calculation = self._normalize_calculation(context)
        if not current_sub_goal and not current_calculation:
            return None

        repeated_sub_goal = False
        repeated_calculation = False
        for action in self.memory.get_actions().values():
            if current_sub_goal and self._normalize_planner_text(action.get("sub_goal")) == current_sub_goal:
                repeated_sub_goal = True
            if current_calculation and self._normalize_calculation(action.get("calculation")) == current_calculation:
    ...
    @staticmethod
    def _make_repeat_judge(repeat_reason: str) -> str:
        return (
    ...
    @staticmethod
    def _call_with_optional_json_data(func, *args, json_data=None):
        if "json_data" in inspect.signature(func).parameters:
            return func(*args, json_data=json_data)
        return func(*args)
    ...
    def solve(self, question: str, image_path: Optional[str] = None):
        ...
        if {'final', 'direct'} & set(self.output_types):
            ...
            query_analysis = self._call_with_optional_json_data(
                self.planner.analyze_query,
                question,
                image_path,
                json_data=json_data,
            )
            ...
            while step_count < self.max_steps and (time.time() - query_start_time) < self.max_time:
                ...
                if tool_name is None:
                    ...
                    action_extra["calculation"] = context
                ...
                # Update memory
                self.memory.add_action(step_count, tool_name, sub_goal, command, result, **action_extra)
                memory_actions = self.memory.get_actions()

                # [5] Verify memory (context verification)
                local_start_time = time.time()
                if fixed_context_verification is not None:
                    context_verification = fixed_context_verification
                    conclusion = fixed_conclusion
                    json_data[f"verifier_{step_count}_response"] = fixed_context_verification
                else:
                ...
                context_verification = make_json_serializable_truncated(context_verification)
                self.memory.add_verification(step_count, context_verification)
                memory_actions = self.memory.get_actions()
                if self.verbose:
            ...
            if 'final' in self.output_types:
                final_output = self._call_with_optional_json_data(
                    self.planner.generate_final_output,
                    question,
                    image_path,
                    self.memory,
                    json_data=json_data,
            ...
            if 'direct' in self.output_types:
                direct_output = self._call_with_optional_json_data(
